...
<body>
  ...
  <main>
    ...
    <section>
      <p class="product-img">
        <img
          src="./img/users/<?php echo $value['user_id'];?>/products/<?php echo $value['listing_id'];?>/<?php echo $value['product_name'] ;?>_1.<?php echo $value['img_extension'];?>"
      ...
      <ul>
        <li class="requests<?php echo $value['requests_sum'];?>">
          <form action="receive_request.php" method="post">
            <button type="submit" class="requests-btn" name="requests-btn"
              value="<?php echo $value['listing_id'];?>">購入申請<?php echo $value['requests_sum'];?>件</button>
          </form>
        </li>
